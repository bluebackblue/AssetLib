...
namespace BlueBack.AssetLib.Editor
{
	...
	*/
	public static class LoadTextWithAssetsPath
	{
		/** ロード。
		...
		*/
		public static MultiResult<bool,string> TryLoad(string a_assets_path_with_extention,System.Text.Encoding a_encoding,int a_offset)
		{
			#pragma warning disable 0168
			try{
				return new MultiResult<bool,string>(true,Load(a_assets_path_with_extention));
		...
		/** ロード。UTF8。BOMなし。
			...
			a_assets_path_with_extention		: フルパス。拡張子付き。

		*/
		public static string LoadNoBomUtf8(string a_assets_path_with_extention)
		{
			return Load(a_assets_path_with_extention,new System.Text.UTF8Encoding(false),0);
		}
		...
		*/
		public static MultiResult<bool,string> TryLoadNoBomUtf8(string a_assets_path_with_extention)
		{
			#pragma warning disable 0168
			try{
				return new MultiResult<bool,string>(true,LoadNoBomUtf8(a_assets_path_with_extention));
			}catch(System.IO.IOException t_exception){
				//ＩＯエラー。
				#if(DEF_BLUEBACK_ASSETLIB_ASSERT)
				DebugTool.Assert(false,t_exception);
